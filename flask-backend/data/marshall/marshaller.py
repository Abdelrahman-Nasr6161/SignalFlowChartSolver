...
class SignalFlowGraphSolverMarshaller:
    ...
    def marshall_output(result):
        return {
            'forwardPaths': [forward_path.nodes for forward_path in result['forward_paths']],
            'loops': [loop.nodes for loop in result['loops']],
            'nonTouchingLoops': {k: [[loop.nodes for loop in k_non_touching_loops_combination]
            ...
            'transferFunction': sp.latex(result['transfer_function'])
                                if result['transfer_function'] != sp.nan
                                else '\\text{Undefined}'
        }